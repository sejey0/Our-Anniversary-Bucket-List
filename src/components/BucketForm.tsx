...
export default function BucketForm({ onSubmit, editItem, onCancel }: BucketFormProps) {
  const [title, setTitle] = useState(editItem?.title || '');
  const [description, setDescription] = useState(editItem?.description || '');
  const [category, setCategory] = useState<string>(editItem?.category || 'Personal');
  const [priority, setPriority] = useState<string>(editItem?.priority || 'Medium');
  const [status, setStatus] = useState<string>(editItem?.status || 'Not Started');
        ...
        <div className="flex gap-3 items-start">
          <div className="flex-1">
            <input
              ...
              placeholder={editItem ? 'Edit title...' : '✨ Add a new dream to our list...'}
